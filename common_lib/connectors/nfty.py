...
class NtfyConnector():
    def __init__(self, ntfy_endpoint_link:str):
        self.endpoint_link = ntfy_endpoint_link

    def send_ntfy_notification(self, topic:str, title:str , message:str, priority:int=3, tags=None, auth=None) -> requests.Response:
        """
        ...
        endpoint = f"{self.endpoint_link}/{topic}"
    ...
    def _validate_connection(self, topic: str, auth = None) -> requests.Response:
        """
        ...
        logging.info(f"Validating {self.endpoint_link}...")
        try:
            base_response = requests.get(self.endpoint_link, verify=True, timeout=5)
            if base_response.status_code != 200:
                logging.warn(f"WARNING: URL {self.endpoint_link} returned status code: {base_response.status_code}")
        except requests.exceptions.RequestException as e:
            logging.error(f"ERROR: Cannot connect to base URL '{self.endpoint_link}': {e}")
            raise e
